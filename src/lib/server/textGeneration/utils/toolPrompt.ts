...
export function buildToolPreprompt(tools: OpenAiTool[]): string {
	...
	return [
		...
		`SINGLE TOOL PER TURN: Call at most one tool in each assistant response. Never emit multiple tool calls in the same response, even if they are independent. If several tools are needed, call the single most useful one first, wait for its result, then decide whether another tool is still needed in a later turn.`,
